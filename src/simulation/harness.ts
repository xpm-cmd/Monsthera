...
function buildSchemaSQL(): string {
  const statements = [
    ...
    [
      "CREATE TABLE work_group_tickets (",
      "  id INTEGER PRIMARY KEY AUTOINCREMENT,",
      "  work_group_id INTEGER NOT NULL REFERENCES work_groups(id),",
      "  ticket_id INTEGER NOT NULL REFERENCES tickets(id),",
      "  added_at TEXT NOT NULL,",
      "  wave_number INTEGER,",
      "  wave_status TEXT DEFAULT 'pending'",
      ")",
    ],
    "CREATE UNIQUE INDEX idx_work_group_tickets_unique ON work_group_tickets(work_group_id, ticket_id)",
    "CREATE INDEX idx_work_groups_repo_status ON work_groups(repo_id, status)",
    "CREATE INDEX idx_wgt_wave ON work_group_tickets(work_group_id, wave_number)",
  ];
