...
/** Extract parsed data from an MCP tool result (content[0].text → JSON.parse). */
function extractToolData(toolResult: { ok: boolean; result?: unknown }): Record<string, unknown> {
  ...
  if (Array.isArray(content) && content.length > 0) {
    ...
    if (typeof first?.text === "string") {
      try { return JSON.parse(first.text) as Record<string, unknown>; } catch { /* ok */ }
    }
  }
  // Fallback: maybe it's already a plain object
  return asRecord(raw);
}
...
export async function cmdSwarm(
  ...
  const regResult = extractToolData(regRaw);
...
async function runGenesis(
  ...
  if (!persistResult.ok) {
    insight.error("Failed to create tickets: " + (persistResult.message ?? persistResult.errorCode));
    process.exit(1);
  }
  const persistData = extractToolData(persistResult);
  const ticketIds = asStringArray(persistData.createdTicketIds);
  if (ticketIds.length === 0) {
    insight.error("decompose_goal returned no ticket IDs. Raw: " + JSON.stringify(persistData).slice(0, 200));
    process.exit(1);
  }
  ...
  if (!wgResult.ok) {
    insight.error("Failed to create work group: " + (wgResult.message ?? wgResult.errorCode));
    process.exit(1);
  }
  const wgData = extractToolData(wgResult);
  const groupId = String(wgData.groupId ?? "");
  if (!groupId) {
    insight.error("Work group created but no groupId returned");
    process.exit(1);
  }
...
async function runPlanningPhase(
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const data = extractToolData(raw);
