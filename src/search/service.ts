...
import { mergeResults, applyReranker, rerankForTrust } from "./hybrid-ranker.js";
...
export class SearchService {
  ...
  async search(
    ...
    // The three ranking stages live in hybrid-ranker.ts (E4 split): hybrid
    // merge → optional relevance rerank → trust rerank.
    const merged = mergeResults(bm25Result.value, semanticResult.value, this.config.alpha);
    const relevanceReranked = await applyReranker(query, merged, {
      rerankEnabled: this.config.rerankEnabled,
      reranker: this.reranker,
      rankProfile: this.config.rankProfile,
      logger: this.logger,
    });
    const trustReranked = await rerankForTrust(query, relevanceReranked, {
      knowledgeRepo: this.knowledgeRepo,
      workRepo: this.workRepo,
    });
