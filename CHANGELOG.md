...
- **Opt-in `snapshot_ready` guard on `enrichment → implementation`** (work article `w-y988ky96`, ADR-006). `WorkTemplateConfig` gains `requiresSnapshotForImplementation?: boolean`; only `FEATURE` is opted in by default. When the flag is on, advancing requires a fresh snapshot for the work id whose lockfile hashes match HEAD — `SnapshotService` supplies freshness, `readHeadLockfileHashes` (`node:fs` + `node:crypto`, no shell) supplies the HEAD side. Existing sync guard contract is unchanged; async guards run as a separate pass, and `skipGuard: { reason }` bypasses them with the same audit trail as every other guard.
