...
(no entries yet — file new work here as it lands)

## [3.0.0] — 2026-05-17

**Cognitive handoff sessions feature.** A new subsystem under `src/sessions/` produces a structured handoff article at every session close, surfaces it as a brief teaser at the next session's start, and gives the incoming agent precise + actionable context — paid for by a local Ollama model, with the coding agent contributing ≤500 tokens net per session for protocol participation. This release consolidates the work shipped across PRs [#104](https://github.com/xpm-cmd/Monsthera/pull/104)–[#113](https://github.com/xpm-cmd/Monsthera/pull/113) (six dogfood calibration cycles) plus the alpha.7-line polish that was sitting on `main` unreleased.

### Added

- **Cognitive handoff sessions** — entirely new subsystem (`src/sessions/`, ~3500 LOC). `Session` is a first-class entity with a JSON-frontmatter lifecycle record plus `<id>.facts.json` sidecar. CLI surface: `monsthera session open|close|brief|get|list` (`src/cli/session-commands.ts`). MCP tool wrappers: `session_open`, `session_close`, `session_get`, `session_list`, `session_brief` (`src/tools/session-tools.ts`). Three-tier content model (T1 deterministic facts / T2 local-Ollama narrative / T3 optional agent `--note`); async dispatch returns close to the caller in ~100 ms with the LLM pipeline running detached. Graceful degradation when Ollama is unavailable yields a T1-only article with `quality.degraded=true`. See [ADR-018](docs/adrs/018-cognitive-handoff-sessions.md) for the 7 architectural decisions the implementation depends on.
- **`SessionStart` hook integration** via Phase E of `docs/agent-bootstrap-guide.md` (PR [#102](https://github.com/xpm-cmd/Monsthera/pull/102) infrastructure). New agent sessions auto-receive a `## Monsthera briefing` block with the previous handoff's TL;DR + cross-agent delta. Net token cost to the coding agent: ~200 tokens to read the teaser, optional ~50 tokens for a `--note` at close. Local Ollama absorbs ~3500 in / ~1700 out per session for the narrative.
- **Handoff coverage validator** (`src/sessions/coverage-validator.ts`) — advisory 5-question agent-readiness check appended as a `## Coverage` section to every handoff. The five dimensions (state / intent / executable-action / constraints / verification) map to the questions a cold-start agent has when picking up work. Calibrated across four dogfood rounds (round 4 in #109; rounds 5–6 in #112, #113). Purely advisory; never blocks persistence.
- **`monsthera session brief`** (PR [#109](https://github.com/xpm-cmd/Monsthera/pull/109)) — read-side complement to `session open --teaser-only`. Three depths (`--depth teaser|standard|full`). Resolves by session id OR by `(agentId, repo)`. Optional `--since` for cross-agent delta. Renders an orphan-fallback brief when the handoff article never landed.
- **Worktree-aware read fallback** (PR [#111](https://github.com/xpm-cmd/Monsthera/pull/111)) — `FileSystemSessionRepository` and `FileSystemKnowledgeArticleRepository` accept an optional `fallbackMarkdownRoot` (the main repo's knowledge dir, resolved via `git rev-parse --git-common-dir`). Reads aggregate from both the worktree's own knowledge dir AND the main repo's; writes stay per-worktree (so the feature branch keeps its session history committed alongside the work). `findOpen` deliberately stays primary-only — an "open" session in another worktree is not this worktree's to supersede. Resolves the SessionStart-hook "Starting fresh" bug when a worktree is freshly branched off a state that predates the relevant handoffs.
- **`DefaultFactsExtractor`** (PR [#108](https://github.com/xpm-cmd/Monsthera/pull/108) Phase 3c) — Stage A facts.json now hydrates from real repo state: orchestration events in window, work-articles touched (with phase-at-open vs phase-at-close), knowledge articles updated, commits in window, code touched (per-file linesAdded/Removed), TODO/FIXME signals from the diff. Replaces the in-memory `MinimalFactsExtractor` used by tests.
- **Round-5 prompt improvements** (PR [#112](https://github.com/xpm-cmd/Monsthera/pull/112)) — `buildRetrospectProspectPrompt` gains three rules: preserve identifiers verbatim (PR numbers, commit SHAs, file:line refs, exact symbol names — no generic-phrase substitution); imperative voice in `nextSteps[].action` (Edit/Run/Fix/Verify, not "The next step is to…"); watch-outs from `agentNote` route to `blockers[]` instead of being absorbed into `decisions[]`. Stage D self-eval rubric switched from count proxies (`decisionCount`, `blockerCount`, etc.) to **5-question content coverage** with a specific-vs-generic distinction.
- **Round-6 codeRefs extractor** (PR [#113](https://github.com/xpm-cmd/Monsthera/pull/113)) — extracted `collectCodeRefs` + `collectArticleReferences` to a new module `src/sessions/handoff-extractors.ts` for isolated unit testing. New regex requires path-shape content with no whitespace (so `` `pnpm test foo.test.ts` `` is no longer captured as a "file path"). `:line` suffix now preserved. Extension list extended to `mjs`, `cjs`, `json`, `yml`, `yaml`, `toml`. Defensive `isPathShaped` post-filter rejects any entry with whitespace, parens, brackets, or backticks.

### Fixed

- **`session open` parent discovery works across worktrees** (PR [#111](https://github.com/xpm-cmd/Monsthera/pull/111) follow-up). `SessionService.open` previously called `findLatestClosed(agentId, input.repo)` which filtered sessions where `s.repo === filter.repo` exactly — sessions opened in main had different `repo` values than the current worktree, so the strict filter returned null even with the repo-layer fallback aggregating both dirs. Bypassed via `findMany({ agentId, status: CLOSED })` (agentId-only) since the fallback already scopes to this Monsthera codebase. `findOpen` left untouched (per-worktree by design).
- **Coverage validator and `collectCodeRefs` anchor the `## Facts` strip to line-start** (PR [#113](https://github.com/xpm-cmd/Monsthera/pull/113)). Both used `body.indexOf("## Facts")`, which matched prose mentions ("the `## Facts` section is structural noise"), chopping off everything below the mention. Anchored regex `/^## Facts/m` + `body.search(…)` fixes it. Found via round-6 dogfood — the fix's own commit message was the trigger.
- **Renderer always emits `### Blockers`** (PR [#109](https://github.com/xpm-cmd/Monsthera/pull/109) round 4). When `summary.blockers === []`, `renderWhatHappened` now emits `### Blockers\n\n_(none identified)_` instead of skipping the section. The validator credits the heading; the next agent knows the previous agent actively checked rather than forgot to look. Renderer-level fix preserves the prompt's "do not invent" rule.
- **Default Ollama model swapped** `qwen2.5-coder:7b` → `gemma4:latest` (PR [#109](https://github.com/xpm-cmd/Monsthera/pull/109)). Root cause of Phase 3c's own dogfood landing degraded was the smaller model being too slow on the default 60 s timeout. `gemma4:latest` clears the same prompt in ~30 s with 4–5/5 self-eval scores on rich notes.
- **`runtime-state.json` no longer mutates on read-only operations.** Two paired changes close a paper cut where every `monsthera status` (and every other CLI/MCP call that boots the container) appeared to mutate persistent state on disk: (1) the file moved from `.monsthera/runtime-state.json` to `.monsthera/cache/runtime-state.json`, signalling — and conventionally allowing downstream repos to gitignore — its ephemeral nature; on first write at the new location, the legacy file is deleted so it stops appearing in `git status`. (2) `searchService.fullReindex({ persistState })` gained an explicit flag, defaulting to `true`. The container's bootstrap reindex (the in-memory `SearchRepo` rehydration that fires whenever `shouldBootstrapSearchIndex` is true — i.e. on every CLI call without Dolt) now passes `persistState: false`. As a result, `lastReindexAt` reflects the user's last *explicit* reindex (CLI `monsthera reindex`, MCP `reindex` tool, dashboard reindex, doctor remediation), not the boot-time bootstrap. Read-only commands like `monsthera status` no longer touch the file. Container surfaces the persisted `lastReindexAt` from runtime-state regardless of whether the bootstrap branch fires, so the user-visible status stays accurate.
...
- **`pnpm lint` is clean again.** Nine pre-existing `@typescript-eslint/no-unused-vars` errors across `src/sessions/workspace-resolver.ts`, `tests/unit/sessions/file-repository-fallback.test.ts`, `tests/unit/sessions/service-brief.test.ts`, and `tests/unit/tools/session-tools.test.ts` cleared by dropping the unused imports and removing a dead `const created = …` from the session-tools brief test setup.

### Documentation

- **[ADR-018 — Cognitive handoff sessions](docs/adrs/018-cognitive-handoff-sessions.md)**. Locks seven decisions: (D1) Sessions are first-class entities, not events or work-articles; (D2) three-tier content model with explicit token-payer per tier; (D3) local Ollama with JSON-mode schema-constrained output, Stages B+C combined into one call; (D4) grounding constraint — every LLM citation must resolve to facts.json; (D5) async dispatch via detached subprocess; (D6) coverage validator as advisory 5-question cross-check, complementary to the LLM-eval; (D7) graceful degradation as a first-class output mode. Plus five implementation deviations documented inline at call sites.
- **Cognitive handoff sessions knowledge corpus**: `knowledge/notes/cognitive-handoff-sessions.md` (system design + state), `knowledge/notes/coverage-validator-round-4-calibration.md` (round-4 learnings), three `phase-3c-`, `phase-3d-3e-`, `phase-4a-4b-shipping-non-obvious-learnings.md` notes capturing what surfaced while shipping each phase that isn't in the commit messages.
- **`AGENTS.md`** — portable agent briefing at the repo root for every assistant (Claude Code, Codex, Cursor, Aider, Gemini) that modifies Monsthera. Covers tech stack, the commands you actually need, and the five non-obvious invariants: stream separation (stdout=data, stderr=logs); `.monsthera/cache/` is the only ephemeral location; `lastReindexAt` semantics (user-initiated only); Markdown-as-source-of-truth + the Option A vs B authoring trap (mixing direct file placement with `knowledge create --content` duplicates frontmatter); and `Result<T, E>` over throwing in domain code. Distinct from the gitignored personal `CLAUDE.md` / `GEMINI.md` so it survives a fresh clone.

### Changed

- **Version**: `3.0.0-alpha.7` → `3.0.0-alpha.8` → `3.0.0`. The alpha.8 bump (commit `d0d2507`) shipped inside PR #109 to mark the validator + worktree-fallback work; the 3.0 bump shipped in PR [#110](https://github.com/xpm-cmd/Monsthera/pull/110) alongside ADR-018 to mark the feature as stable and dogfooded.
...
- **`--blocked-by` and `--dependencies` on `work create` and `work update`.** The schema already supported these fields (frontmatter round-trips them through `WorkArticleFrontmatterSchema`), and the repository's `CreateWorkArticleInput` already accepted them — but `CreateWorkArticleInputSchema` (the service-boundary Zod schema) silently stripped them, so the CLI had no way to populate the wave-level dep graph except via free-text tags (`wave-1`, `blocked-by-ws14`). Now each comma-separated id list on `work create` populates its respective frontmatter array after every id has been verified to exist in the repo (dangling references error out before the article is created). On `work update`, both flags route through the existing `addDependency` service method so each add emits an orchestration event and mirrors the MCP `add_dependency` tool's audit trail exactly. The help block codifies the nuance that `addDependency` maintains `blockedBy ⊆ dependencies`, so `--dependencies` on `update` behaves identically to `--blocked-by`.
- **`--no-imported-tag` on `ingest local`** (and the `noImportedTag: boolean` field on the `ingest_local_sources` MCP tool). `ingest local` has always unconditionally appended `imported` to the tag set of every article it creates, which conflicts with consumers who want the imported corpus to look identical to a hand-authored corpus (for filtering, wiki-index grouping, etc.). The flag is an opt-out, not a breaking change — default behaviour is unchanged, so existing pipelines keep their tags.

### Documentation

- **`docs/concurrency-model.md`** — research note documenting the last-write-wins behaviour of every work-article mutation (`contributeEnrichment`, `assignReviewer`, `submitReview`, `addDependency`, `removeDependency`, `updateWork`, `advancePhase`). The investigation was triggered by the question "is `work enrich` atomic when two subagents contribute simultaneously?" and verified by `scripts/race-enrich.ts`, an ad-hoc in-process repro that shows **20/20 rounds lose one write** when two `contributeEnrichment` calls share a container. CLI invocations are effectively serialised by process-boot latency (0/10 losses observed) but that is a timing accident, not a guarantee. The note outlines three fix directions (role-level guard, optimistic concurrency via `updatedAt`, or a keyed async mutex in the service layer) without implementing any of them — per the Tier 7 brief, the fix deserves its own ADR and PR. Consumers should treat every work-article mutation as single-writer-per-article until then.
- **`docs/migration-from-v2.md`** — the *how* companion to ADR-002. One-row-per-primitive map from v2 concepts (`decompose_goal`, `create_ticket`, `assign_council`, `submit_verdict`, `check_consensus`, `compute_waves`, `launch_convoy`, `add_protected_artifact`, `export_audit`) to v3 equivalents, plus sections on idioms that no longer translate (quorum voting, absolute vetoes, waves-as-first-class) and worked examples for re-modelling a council, a wave, a veto, and an audit export against the v3 surface.
- **`docs/consumer-setup.md`** — onboarding guide for downstream repos. Six steps: clone + build Monsthera, point the consumer repo's `.mcp.json` at the built `dist/bin.js` (not the dev `tsx src/bin.ts` path), sanity-check via `status --repo .`, enable Ollama + `nomic-embed-text` for semantic search, first `knowledge create` / `work create`, and the upgrade loop. Plus a troubleshooting section covering the three most common external-onboarding issues: MCP client not reloading, wrong `--repo` defaulting to the client's cwd, and stdout/stderr mixing when piping.
...
- **Regression test for CLI stdout/stderr separation.** The structured JSON logger was already routing INFO/WARN/DEBUG/ERROR to stderr by default, but there was no test guarding this contract against future regressions. A new `tests/integration/cli-stream-separation.test.ts` spawns `dist/bin.js` in a child process and asserts that (a) `knowledge list` keeps structured log entries out of stdout, and (b) `status` emits a single parseable JSON document on stdout with the logger output landing on stderr — so downstream scripts can safely `2>/dev/null` and pipe the result.
