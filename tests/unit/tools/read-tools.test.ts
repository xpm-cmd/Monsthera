...
describe("read tool discovery", () => {
  ...
  it("describes required actor fields for protected knowledge and coordination tools", async () => {
    ...
    expect(JSON.parse(runWorkflow.content[0].text).inputSchema).toMatchObject({
      name: "string (built-in: onboard|deep-review|ta-review|deep-review-v2|backlog-triage|auto-resolve|take-job; custom: custom:<name>)",
...
`,
    ...
    expect(schemaPayload.inputSchema).toMatchObject({
      name: "string (built-in: onboard|deep-review|ta-review|deep-review-v2|backlog-triage|auto-resolve|take-job; custom: custom:repo-review)",
    });
  });
